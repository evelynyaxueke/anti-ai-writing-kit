...
function readCandidate(options, io) {
  ...
  if (options.stdin && !injectedStdin && stdinIsTty) {
    throw new GateUsageError('--stdin requires complete framed input to be attached when the process starts. Interactive terminal input is not supported.');
  }
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--stdin') {
      ...
      if (arg === '--input') options.input = value;
        ...
        if (!/^AAW\d{3}@\d+(?:,AAW\d{3}@\d+)*$/u.test(value)) {
          throw new GateUsageError('--allow-review requires comma-separated occurrence IDs such as AAW012@35.');
        }
...
function privateTextOutput(result, limitFailure, allowedReview) {
  ...
  lines.push('MECHANICAL PASS IS NOT DELIVERY PASS. Privately run the one-answer paragraph test, cross-section primary-location check, reader-trust deletion pass, source-silence and relationship-scope checks, all/every quantifier check, outcome-by-outcome coverage check, future-causal-proof check, capability-promotion check, restatement-label check, recommendation-once check, and first/last paragraph check. Rerun this gate after any edit.');
