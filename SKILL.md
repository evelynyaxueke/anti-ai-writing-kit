...
4. Use interactive `--stdin` only when private temporary-file input is unavailable and the runtime provides a writable process session. Start the checker, wait for its session handle, send the complete candidate followed by the exported `CANDIDATE_INPUT_EOF_MARKER`, then close input. Never launch `--stdin` as an ordinary blocking shell command with no attached input. If the runtime provides neither a private temporary-file writer nor a writable process session, do not launch the checker; complete the semantic review and visible scan manually. Never embed the candidate in `printf`, `echo`, a shell command, an environment variable, a here-document, or a command-line argument.
