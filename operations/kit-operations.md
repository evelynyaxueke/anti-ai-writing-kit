...
2. If private file input is unavailable, use interactive stdin only through a writable process session. Start the checker, wait for its session handle, send the candidate followed by exactly `\n__ANTI_AI_CANDIDATE_INPUT_EOF__\n`, then close input. Never launch the stdin mode as an ordinary blocking shell command with no attached input. If the runtime provides neither a private temporary-file writer nor a writable process session, do not launch the checker; complete the semantic review and visible scan manually.
