...
2. If private file input is unavailable, use `--interactive-stdin` only through a writable process session. Start the checker, wait for its session handle, send the candidate followed by exactly `\n__ANTI_AI_CANDIDATE_INPUT_EOF__\n`, then close input. Plain `--stdin` accepts only framed bytes attached when the process starts and rejects an interactive terminal. If the runtime provides neither a private temporary-file writer nor a writable process session, do not launch the checker; complete the semantic review and visible scan manually.
