...
Keep candidate binding honest. The runtime workflow should prefer a fresh mode-`0600` temporary file and use `--interactive-stdin` only through a writable process session. Plain `--stdin` must reject an interactive terminal before reading. Verify the latest receipt's `candidate_sha256` against the exact candidate and require a full rerun after any character change. Without runtime or harness comparison, documentation must not claim that the receipt proves final assistant-message equality.
